...
const ScheduleMain = () => {
  ...
  return (
    <>
      <h1 className="text-muted mb-3">SCHEDULES</h1>
      <div className="row">
                ...
                {theaters.map((theater, index) => (
                  <div className="col-md-3 mb-4" key={index}>
                    <div className="card border-success border">
                      <div className="card-body">
                        <h5 className="card-title text-success">{theater.name}</h5>
                        <p className="card-text">ID:{theater.id}</p>
                        <p className='card-text'>Address: {theater.address}</p>
                        <a href={`/admin/detail-schedule/${theater.id}`} class="btn btn-success btn-sm">Check Schedule</a>
                      </div>
                    </div>
                  </div>
                ))}
